...
def getProLink(db):
	...
	for i in range(0,pageNum):
		...
		statusinfos = curSoup.findAll('div',{'class':'progress-wrap'})
		proinfos = curSoup.findAll('li',{'class':'product-list clearfix '})
		amountinfos = curSoup.findAll('div',{'class':'product-amount is-3rd-col'})
		for i in range(0,len(statusinfos)):
...
def getProDetail(db,proinfo,statusinfo,amountinfo):
	...
	#print record_obj
	write_record_db(db,record_obj,'p2p_product_lufax_zhuanxiang')
...
if __name__ == '__main__':
	...
	os.system(script_path + '  p2p_product_lufax_zhuanxiang')
	#os.system('/home/dong/p2p3000/tool/empty_db_table.sh  p2p_product_lufax_zhuanxiang')
	getProLink(db)
